...
- **Bounded file bundles** use one version-1 JSON envelope for the fixed `html-css` (`index.html` + `style.css`), `html-js`/`html-js-fetch` (`index.html` + `script.js`), `html-css-js` (`index.html` + `style.css` + `script.js`), `html-js-css-media` (the same three code files plus host UI), and `react-app` schemas. React App requires `App.jsx`, permits up to two lesson-defined top-level component JSX files plus `App.css`, resolves exact local component imports with a per-run CommonJS cache, and auto-renders the entry's default export. Plain legacy code becomes the configured entry file. Runtime recipes contain inline parsers because iframe logic cannot import modules.
...
- **Fetch-enabled modes** receive trusted host-owned `mockApi` routes in `EXECUTE.payload`, replace `window.fetch` with an abortable `Response`-based mock, and run learner code with top-level `await`. Routes can constrain exact queries, required header subsets, and deep-equal JSON request bodies; the most specific match wins. `html-js-fetch` shares the bounded `html-js` renderer and adds the read-only API Server tab. Routes never enter persisted code or `srcDoc`; `connect-src 'none'` prevents real connection APIs, and iframe permissions remain unchanged. Mock API keys are visible lesson fixtures, never secrets.
