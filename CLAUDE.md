...
- **Bounded file bundles** use one version-1 JSON envelope for the fixed `html-css` (`index.html` + `style.css`), `html-js`/`html-js-fetch` (`index.html` + `script.js`), `html-css-js` (`index.html` + `style.css` + `script.js`), `html-js-css-media` (the same three code files plus host UI), and `react-app` (`App.jsx` + `App.css`) schemas. Plain legacy code is assigned to the configured first file. The runtime recipes contain small inline parsers because iframe logic cannot import modules. React App applies CSS only for the exact `import './App.css'` specifier and auto-renders the default export.
