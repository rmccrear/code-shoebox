 ...
 * This ensures that all public environment modes are recognized as valid modes in the runtime,
 * resolving "is not assignable" errors in OutputFrame.tsx and ServerOutput.tsx.
 */
export type EnvironmentMode = 'dom' | 'html-css' | 'p5' | 'p5-ts' | 'react' | 'typescript' | 'react-ts' | 'express' | 'express-ts' | 'node-js' | 'node-ts' | 'hono' | 'hono-ts';
